...
const STORAGE_KEY = "infinox-smart-insights";
const STALE_MIN = 30;
...
const SmartInsights = () => {
  ...
  const load = async (force = false) => {
    ...
    try {
      ...
      toast({ title: "Insights unavailable", description: e?.message || "Try again later.", variant: "destructive" });
    } finally {
      setLoading(false);
    }
  };
      ...
      <header className="mb-4 flex items-center justify-between gap-3">
        <div className="flex items-center gap-2.5 min-w-0">
          ...
          <div className="min-w-0">
            <h3 className="font-heading text-sm font-bold text-foreground tracking-tight">AI Smart Insights</h3>
      ...
      {isLoading ? (
        <div className="space-y-4">
          <div className="flex items-center gap-2 text-xs text-muted-foreground">
            <Loader2 className="h-3.5 w-3.5 animate-spin text-primary" /> Analyzing market with AI…
          </div>
        ...
        <div className="space-y-5">
          {/* Market summary */}
          <div>
            <p className="mb-1.5 font-mono text-[10px] uppercase tracking-wider text-primary/80">Market Summary</p>
          ...
          {data.key_opportunities?.length > 0 && (
            <div>
              <p className="mb-2 font-mono text-[10px] uppercase tracking-wider text-primary/80">Key Opportunities</p>
          ...
          {data.risk_alerts?.length > 0 && (
            <div>
              <p className="mb-2 font-mono text-[10px] uppercase tracking-wider text-primary/80">Risk Alerts</p>
        ...
        <p className="py-8 text-center text-sm text-muted-foreground">Insights unavailable. Tap refresh to retry.</p>
      )}
    </section>
  );
};
